...
type InboxResponse = { messages: MailItem[]; result_size_estimate?: number };
...
export default function InboxPage() {
  ...
  return <WorkspaceFrame title="Gmail Inbox" eyebrow="Your connected mailbox">
    <main className="standalone-page"><div className="content">
      {!connected ? <section className="panel empty-loading"><Mail size={30} /><div><h2>Connect your Gmail account</h2><p>Each user connects their own Gmail account through Google OAuth. Your password is never stored in this system.</p><button className="primary-button" onClick={connect}><Link2 size={16} /> Connect Gmail</button></div></section> : <>
        <div className="page-intro"><div><p className="eyebrow">{status.data.email || "Connected Gmail"}</p><h2>Inbox</h2><p>Read and send messages without leaving the sales workspace.</p></div><div className="top-actions"><button className="secondary-button" onClick={() => queryClient.invalidateQueries({ queryKey: ["gmail"] })}><RefreshCw size={16} /> Refresh</button><button className="secondary-button" onClick={async () => { await apiClient.delete("/v1/google/disconnect"); queryClient.invalidateQueries({ queryKey: ["google-status"] }); }}><Unplug size={16} /> Disconnect</button></div></div>
        {notice && <div className="notice"><Sparkles size={16} /> {notice}<button onClick={() => setNotice("")}><X size={15} /></button></div>}
        <section className="panel"><div className="mail-toolbar"><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Gmail search, e.g. is:unread" /><button className="primary-button" onClick={() => { setSelectedId(null); setComposer(true); setBody(""); setTo(""); setSubject(""); }}><Mail size={16} /> New email</button></div><div className="inbox-layout"><div className="mail-list">{inbox.isLoading ? <div className="empty">Loading messages…</div> : messages.length === 0 ? <div className="empty">No messages found.</div> : messages.map(message => <button className={selectedId === message.id ? "mail-row selected" : `mail-row${message.is_unread ? " unread" : ""}`} key={message.id} onClick={() => openMessage(message.id)}><span className="mail-sender">{displayName(message.from)}</span><span className="mail-subject">{message.subject}</span><span className="mail-snippet">{message.snippet}</span><time>{message.date ? new Date(message.date).toLocaleDateString() : ""}</time></button>)}</div><div className="mail-detail">{composer ? <form onSubmit={submit} className="mail-compose"><div className="panel-head"><div><p className="eyebrow">New message</p><h3>Compose email</h3></div></div><label>To<input required type="email" value={to} onChange={e => setTo(e.target.value)} placeholder="client@example.com" /></label><label>Subject<input required value={subject} onChange={e => setSubject(e.target.value)} placeholder="Subject" /></label><label>Message<textarea required value={body} onChange={e => setBody(e.target.value)} placeholder="Write your message or generate an AI draft…" /></label><div className="modal-actions"><button type="button" className="secondary-button" onClick={() => draft.mutate()} disabled={draft.isPending || !selected.data}>{draft.isPending ? "Generating…" : "Generate AI draft"}</button><button className="primary-button" disabled={send.isPending || !body}>{send.isPending ? "Sending…" : <><Send size={15} /> Send</>}</button></div></form> : selected.data ? <><div className="panel-head"><div><p className="eyebrow">{selected.data.from}</p><h3>{selected.data.subject}</h3><small>{selected.data.date ? new Date(selected.data.date).toLocaleString() : ""}</small></div><button className="primary-button" onClick={() => { setComposer(true); setTo(address(selected.data?.from)); setSubject(selected.data?.subject?.toLowerCase().startsWith("re:") ? selected.data.subject : `Re: ${selected.data?.subject || ""}`); setBody(""); }}>Reply</button></div><article className="mail-body">{selected.data.body || selected.data.snippet}</article><div className="ai-action"><Sparkles size={18} /><div><strong>AI reply assistant</strong><p>Generate a draft, review it, edit anything needed, then send it yourself.</p></div><button className="secondary-button" onClick={() => { setComposer(true); setTo(address(selected.data?.from)); setSubject(selected.data?.subject?.toLowerCase().startsWith("re:") ? selected.data.subject : `Re: ${selected.data?.subject || ""}`); draft.mutate(); }} disabled={draft.isPending}>{draft.isPending ? "Generating…" : "Generate draft"}</button></div></> : <div className="empty"><Mail size={28} /><span>Select a message to read it.</span></div>}</div></div></section>
      </>}
    </div></main>
  </WorkspaceFrame>;
}
