...
export default function LinkedInPage() {
  ...
  useEffect(() => { load(); }, []);
  async function connect() { setBusy(true); try { const r = await apiClient.get("/v1/linkedin/connect"); window.location.href = r.data.url; } catch (e: any) { setMessage(e?.response?.data?.message || "LinkedIn OAuth is not configured."); setBusy(false); } }
