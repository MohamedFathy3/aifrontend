...
export function CrmRecordsPage({ type }: { type: CrmType }) {
  ...
  return <WorkspaceFrame title={page.title} eyebrow={page.eyebrow}>
    ...
    {editRow && <EditRecordModal type={type as "clients" | "leads" | "shipments"} row={editRow} loading={update.isPending} onClose={() => setEditRow(null)} onSubmit={(payload) => update.mutate({ id: editRow.id, payload })} />}
  </WorkspaceFrame>;
}
