...
import { ArrowUpRight, Bot, Building2, CalendarDays, ClipboardList, LayoutDashboard, Linkedin, Mail, Menu, MessageCircle, Search, Ship, Users, X } from "lucide-react";
...
export function WorkspaceFrame({ title, eyebrow, children }: { title: string; eyebrow: string; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full flex-col border-r border-slate-200 bg-white p-4 shadow-xl transition-transform lg:translate-x-0 lg:shadow-none", open && "translate-x-0")}>
      ...
      <nav className="grid gap-1">{links.map(({ href, label, icon: Icon }) => { const active = href.includes("?") ? pathname === "/dashboard" && new URLSearchParams(typeof window !== "undefined" ? window.location.search : "").get("tab") === href.split("=")[1] : pathname === href && !links.some(item => item.href.includes("?") && pathname === item.href.split("?")[0]); return <Link href={href} key={href} onClick={() => setOpen(false)} className={cn("group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900", active && "bg-teal-50 text-teal-700")}>{<Icon size={17} className={cn(active ? "text-teal-600" : "text-slate-400 group-hover:text-slate-700")} />}<span>{label}</span>{active && <ArrowUpRight className="ml-auto" size={14} />}</Link>; })}</nav>
