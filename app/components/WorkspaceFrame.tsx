...
import { ArrowUpRight, Bot, Building2, CalendarClock, CalendarDays, ChevronLeft, ChevronRight, ClipboardList, LayoutDashboard, Linkedin, Mail, MapPinned, Menu, MessageCircle, Plus, Search, Ship, Users, X } from "lucide-react";
...
const groups: NavGroup[] = [
  ...
  },
  {
    ...
    links: [
      ...
      { href: "/scheduled-emails", label: "Scheduled emails", icon: CalendarClock },
  ...
  {
    label: "Integrations",
    links: [{ href: "/linkedin", label: "LinkedIn", icon: Linkedin }],
  },
];
...
export function WorkspaceFrame({ title, eyebrow, children }: { title: string; eyebrow: string; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full flex-col border-r border-slate-200 bg-white p-4 shadow-xl transition-all duration-200 lg:translate-x-0 lg:shadow-none", collapsed ? "lg:w-[76px]" : "lg:w-64", open && "translate-x-0 w-64")}>
      <div className={cn("flex shrink-0 items-center gap-3 px-2 py-3", collapsed && "lg:justify-center lg:px-0")}><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-900 text-lg font-black text-white shadow-lg shadow-slate-900/15">P</div><div className={cn(collapsed && "lg:hidden")}><p className="font-bold tracking-tight text-slate-900">Pyramidth</p><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-slate-400">Sales CRM</p></div><button className="ml-auto rounded-lg p-2 text-slate-400 hover:bg-slate-100 lg:hidden" onClick={() => setOpen(false)} aria-label="Close navigation"><X size={18} /></button></div>
      <div className="my-4 h-px shrink-0 bg-slate-100" />
      <nav className="min-h-0 flex-1 overflow-y-auto pr-1">{groups.map(group => <section key={group.label} className="mb-5"><p className={cn("px-3 pb-2 text-[10px] font-bold uppercase tracking-[.16em] text-slate-400", collapsed && "lg:hidden")}>{group.label}</p><div className="grid gap-1">{group.links.map(({ href, label, icon: Icon }) => { const active = isActive(href); return <Link href={href} key={href} title={collapsed ? label : undefined} onClick={() => setOpen(false)} className={cn("group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900", collapsed && "lg:justify-center lg:px-0", active && "bg-teal-50 text-teal-700")}><Icon size={17} className={cn(active ? "text-teal-600" : "text-slate-400 group-hover:text-slate-700")} /><span className={cn(collapsed && "lg:hidden")}>{label}</span>{active && <ArrowUpRight className={cn("ml-auto", collapsed && "lg:hidden")} size={14} />}</Link>; })}</div></section>)}</nav>
      <div className={cn("mt-3 shrink-0 rounded-xl bg-slate-50 p-3", collapsed && "lg:bg-transparent lg:p-0")}><div className={cn("flex items-center gap-3", collapsed && "lg:justify-center")}><div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-teal-100 text-teal-700"><Bot size={17} /></div><div className={cn(collapsed && "lg:hidden")}><p className="text-xs font-semibold text-slate-800">Sales workspace</p><p className="text-[11px] text-slate-400">AI tools enabled</p></div></div></div>
    </aside>
